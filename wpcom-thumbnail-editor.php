...
Plugin Name:  WIRED's WordPress.com Thumbnail Editor
...
Author:       Automattic | Zack Tollman
...
class WPcom_Thumbnail_Editor {
	...
	function __construct() {
		...
		/**
		 * WordPress.com relies on Photon to resize the image on the fly. Rather than use the crop information to alter
		 * the image on the server, WordPress.com relies on a special image URL being returned, which will grab a cropped,
		 * on the fly version of the image. Since this is irrelevant for WIRED's Photon-less world, the filter is removed.
		 * In fact, with the filter in place, the full size image is always returned for the images.
		 */
		...
		if ( is_admin() ) {
			...
			// Dropping the admin notice since we aren't using Photon on Wired.
			// @since 10/24/2014
			// @whyisjake
	...
	public function get_attachment_field_html( $attachment ) {
		...
		// Pagenow is needed to see if we should loop all of the thumbs, and whether or now to show the link to all images.
		global $pagenow;

		$html = '<div id="' . esc_attr( 'wpcom-thumbs-' . $attachment->ID ) . '" class="">';
			...
			foreach ( $sizes as $key => $size ) {
				...
				if( function_exists( 'jetpack_photon_url' ) )
					...
					$thumbnail_url = apply_filters( 'wpcom_thumbnail_editor_url', $thumbnail[0] );

				$dimensions   = $this->get_thumbnail_dimensions( $image_name );
				$display_name = ( isset( $dimensions['width'], $dimensions['height'] ) ) ? absint( $dimensions['width'] ) . 'x' . absint( $dimensions['height'] ) : '';


				if ( $size == 'thumbnail' || $pagenow == 'media.php' ) {
					$html .= '<div style="float:left;margin:0 20px 20px 0;min-width:250px;">';
						...
						$html .= '>';
							$html .= '<strong>' . esc_html( $display_name ) . '</strong><br />';
			...
			if ( $pagenow !== 'media.php' ) {
				$html .= '<p><a href="'. esc_url( admin_url( 'media.php?action=edit&attachment_id=' . $attachment->ID ) ) . '" target="_blank" class="button">' .  __( 'See All Crops', 'wpcom-thumbnail-editor' ) . '</a></p>';
			}
...
<?php
	...
	public function post_handler() {
		...
		/**
		 * Because WIRED cannot rely on Photon to handle the image resizing. Instead, the resize needs to occur when the
		 * image is cropped in the admin. By sending the crop information, the new image is created, which replaces the
		 * original thumbnail image.
		 */
		wired_crop_image(
			$attachment->ID,
			$fullsize_selection_x1,
			$fullsize_selection_y1,
			( $fullsize_selection_x2 - $fullsize_selection_x1 ),
			( $fullsize_selection_y2 - $fullsize_selection_y1 ),
			$_REQUEST['size']
		);
	...
	public function get_intermediate_image_sizes( $cropped_only = true ) {
		...
		// Remove usage of filters not available outside of WordPress.com
		...
		/**
		 * Ultimately, the image sizes need to be sorted by their actual dimensions. In order to do so, we need to take
		 * the array of image size names and collect the dimensions of the images so we can use that for sorting.
		 * $sizes_temp holds the mapping of image names to image sizes.
		 */
		$sizes_temp = array();
		foreach ( $sizes as $size ) {
			$sizes_temp[ $size ] = $this->get_thumbnail_dimensions( $size );
		}

		/**
		 * Now that the sizes are available, sort them. The images should be displayed in order of width then height:
		 *
		 *   - 200x100
		 *   - 200x300
		 *   - 300x100
		 */
		uasort( $sizes_temp, function ( $a, $b ) {
			$width_a  = (int) $a['width'];
			$width_b  = (int) $b['width'];
			$height_a = (int) $a['height'];
			$height_b = (int) $b['height'];
		...
		/**
		 * Now that $sizes_temp holds a mapping of image sizes to image dimensions sorted in the order that we wish, get
		 * the keys for the array. This will give us the appropriate ordering of image sizes in the appropriate format
		 * for the this function.
		 */
		return array_keys( $sizes_temp );
	}
...
/**
 * Crop and replace an image for a specific image size.
 *
 * @param  int       $post_id                  The image ID.
 * @param  int       $fullsize_selection_x1    The upper left hand X coordinate add -pto begin the crop from.
 * @param  int       $fullsize_selection_y1    The upper left hand Y coordinate to begin the crop from.
 * @param  int       $crop_width               The X distance from the origin for the crop.
 * @param  int       $crop_height              The Y distance from the origin for the crop.
 * @param  string    $size                     The named image size.
 * @param  string    $file_name                The name for the resulting image.
 * @return array                               Information about the newly created image.
 */
function wired_crop_image( $post_id, $fullsize_selection_x1, $fullsize_selection_y1, $crop_width, $crop_height, $size, $file_name = '' ) {
	$post = get_post( $post_id );
	$meta = wp_get_attachment_metadata( $post_id );

	// Load the image into the image editor to prepare for manipulation
	$img  = wp_get_image_editor( _load_image_to_edit_path( $post_id, 'full' ) );

	if ( ! is_wp_error( $img ) ) {
		if ( ! empty( $meta['sizes'][ $size ]['file'] ) && ! empty( $meta['sizes'][ $size ]['height'] ) && ! empty( $meta['sizes'][ $size ]['width'] ) ) {
			// Use this image's file name, width and height to size and name the new image
			$image_height = $meta['sizes'][ $size ]['height'];
			$image_width  = $meta['sizes'][ $size ]['width'];

			/**
			 * Crop the image. Start by sending the origin X, Y point. Then send the X and Y distance from that origin.
			 * Finally, reduce the image size once the crop is done to the size of the original image.
			 */
			$img->crop( $fullsize_selection_x1, $fullsize_selection_y1, $crop_width, $crop_height );

			// Generate new filename.
			$path          = get_attached_file( $post_id );
			$path_parts    = pathinfo( $path );
			$filename_base = $path_parts['filename'];
			$size_slug     = $image_width . 'x' . $image_height;
			$salt          = 'e' . time() . rand( 100, 999 );
			$extension     = $path_parts['extension'];
			$new_filename  = ( empty( $file_name ) ) ? $filename_base . '-' . $size_slug . '-' . $salt . '.' . $extension : $file_name;
			$new_path      = $path_parts['dirname'] . '/' . $new_filename;

			// Save the image to the specified location with the specified mime type
			$img->save( $new_path, $post->post_mime_type );

			// Get the old filename to determine if an update is needed
			$old_filename = ( ! empty( $meta['sizes'][ $size ]['file'] ) ) ? $meta['sizes'][ $size ]['file'] : '';

			if ( $old_filename !== $new_filename ) {
				// Update the image metadata with the new file name
